...
export async function handleEvent(event: DomainEvent) {
  ...
  if (!rule) {
    console.log(`No notification rule found for event type: ${event.type}`);
    return;
  }

  try {
    const receivers = await rule.resolveReceivers(event);
    console.log(`Found ${receivers.length} receivers for ${event.type} event`);
    ...
    for (const receiverId of receivers) {
      ...
      if (notification) {
        ...
        console.log(`Sent SSE notification to ${receiverId.type}_${userId} for ${event.type}`);
      }
    }
  } catch (error) {
    console.error(`Failed to handle ${event.type} event:`, error);
  }
}
